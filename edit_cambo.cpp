...
#include <string>
#include <cstdlib>
#include <algorithm>   // For formatting output
...
void saveUsers() {
    ...
    for (const auto& user : users) {
        file << user.username << ' ' << user.email << ' '<< user.phone << ' ' << user.password << endl;
    }
}
...
//=======================================================================================================================================
   ...
   if (!isValidEmail(email)) {
        cout << "Error: Invalid email format. Please use '@email.com'." << endl;
    ...
    users.push_back({username, email, phone,  password});
...
void showLoading() {
    ...
    for (int i = 0; i < 5; ++i) {
        cout << ".";
        cout.flush(); // Ensure the dots are shown immediately
        this_thread::sleep_for(chrono::milliseconds(500)); // Wait 0.5 seconds between dots
    }
    cout << endl;
}

// Function to recover a forgotten password by email or phone with loading simulation
void forgotPassword() {
    cout << " |-----------------------------------------------------------|" << endl;
    ...
    int option;
    cout << "Choose recovery method:" << endl;
    cout << " 1. By Email" << endl;
    cout << " 2. By Phone Number" << endl;
    cout << "Please select an option: ";
    cin >> option;

    string identifier; // This will store either email or phone number based on the user's choice
    ...
    if (option == 1) {
        ...
        cout << "Phone number not found! Please try again." << endl;
...
void changePassword() {
    ...
    int option;
    cout << "Choose verification method to change password:" << endl;
    cout << " 1. By Email" << endl;
    cout << " 2. By Phone Number" << endl;
    cout << "Please select an option: ";
    cin >> option;

    string identifier; // Stores either email or phone number as string
    if (option == 1) {
        cout << "Enter your registered email: ";
        cin >> identifier;
        ...
        cout << "Enter your registered phone number: ";
        cin >> identifier; // Treat phone as a string

        // Find user by phone number
        for (auto& user : users) {
            if (user.phone == identifier) { // Both phone and identifier are strings now
                string old_password;
                cout << "Enter old password: ";
                cin >> old_password;
...
//======================================================================================================================================
 void all(){
    ...
    while (true)
    {
        ...
        printCenteredWithTabs("Enter |c| to search by category.", consoleWidth);
        printCenteredWithTabs("Enter |v| to view cart.\t\t\t\t\t\t\t\t", consoleWidth);
        printCenteredWithTabs("Enter |b|checkout and buy\t\t\t\t\t.", consoleWidth);
